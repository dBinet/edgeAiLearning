...
model_loaded = make_3layer()

# train and save model
train(model_3l)
torch.save(model_3l.state_dict(), 'models/wine_3layer.pth')

# Load weights into a new model
model_loaded.load_state_dict(torch.load('models/wine_3layer.pth'))  

model_3l.eval()
model_loaded.eval()
with torch.no_grad():
    out_original = model_3l(X_test_t)
    out_reloaded = model_loaded(X_test_t)
    diff = (out_original - out_reloaded).abs().max().item()
    print(f"Max output diff: {diff}")
    evaluate(model_3l)
